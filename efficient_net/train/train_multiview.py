...
def build_model(dropout: float = 0.4, model_name: str = "efficientnet_b0") -> nn.Module:
...
def make_optimizer(model: nn.Module, lr: float, phase: int) -> torch.optim.Optimizer:
    if phase == 1:
        return torch.optim.AdamW(model.classifier.parameters(), lr=lr * 10, weight_decay=1e-4)
    ...
    return torch.optim.AdamW(
        [
            ...
            {"params": model.classifier.parameters(), "lr": lr * 10, "weight_decay": 1e-4},
        ]
    )
...
def train_view_model(
    ...
    model     = build_model(args.dropout, args.model_name).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch == args.unfreeze_epoch and phase == 1:
            print(
                ...
                f"backbone lr={args.lr:.1e}, classifier lr={args.lr*10:.1e}"
            )
